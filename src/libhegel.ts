 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.41.0) via {@link koffi}.
 *
 ...
 * absorbed as NULL (engine output stays on stderr). Two further parameters are
 * absorbed with behavior-preserving defaults the client never varies: the
 * `forced` / `has_forced` pair on `hegel_generate_boolean` (both `false` — the
 * client never forces a draw), and the optional `alphabet` generator on
 * `hegel_string_generator_regex` (NULL — the pattern alone defines the alphabet).
 */
