 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.37.10) via {@link koffi}.
 *
 ...
 * absorbed as NULL (engine output stays on stderr), as is
 * `hegel_string_generator_regex`'s optional `alphabet` argument (NULL — the
 * client never constrains the pattern's alphabet), the `forced` / `has_forced`
 * pair of `hegel_generate_boolean` (`false, false` — the client never forces a
 * draw), and the `user_data` passed alongside each absorbed callback.
 */
