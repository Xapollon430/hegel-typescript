 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.36.4) via {@link koffi}.
 *
...
export const RunStatus = {
  ...
  FAILED: 1,
  ERROR: 2,
  // A run declared nondeterministic (a test case created a state machine with
  // `max_concurrency > 1`) that then failed. Unreachable for this client: it
  // binds no state-machine functions, so it never declares a run
  // nondeterministic. Mirrored here to keep the enum faithful to the ABI.
...
/**
 * A `hegel_time_t`: a time of day with nanosecond precision (`nanosecond` in
 * `[0, 999999999]`; it was microsecond precision before libhegel 0.36).
 */
