 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.37.3) via {@link koffi}.
 *
 ...
 * absorbed as NULL (engine output stays on stderr), as are two optional draw
 * parameters this client never uses: `hegel_generate_boolean`'s
 * `forced` / `has_forced` (absorbed as `false, false` — no forced value) and
 * `hegel_string_generator_regex`'s `alphabet` (absorbed as NULL — no alphabet
 * constraint).
 */
