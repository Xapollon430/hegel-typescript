 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.36.5) via {@link koffi}.
 *
 ...
 * absorbed as NULL (engine output stays on stderr), as are the optional handle
 * arguments this client never supplies: `hegel_string_generator_regex`'s
 * `alphabet` (no custom alphabet). {@link bindLibrary} also absorbs
 * `hegel_generate_boolean`'s `forced` / `has_forced` pair as `false, false` —
 * the client never forces a boolean draw, so booleans are always drawn freely.
 */
