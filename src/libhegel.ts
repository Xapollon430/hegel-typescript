 ...
 * `hegel-rust/hegel-c/include/hegel.h`, version 0.37.4) via {@link koffi}.
 *
 ...
 * absorbed as NULL (engine output stays on stderr), as are
 * `hegel_generate_boolean`'s `forced` / `has_forced` pair (the client never
 * forces a draw) and `hegel_string_generator_regex`'s `alphabet` (always the
 * engine default).
 */
